      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 w-full max-w-xs bg-white dark:bg-slate-900 shadow-2xl z-[101] flex flex-col"
          >
            {/* Header */}
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-slate-900 dark:bg-primary-blue rounded-xl flex items-center justify-center">
                  <Bell className="text-white w-5 h-5" />
                </div>
                <div>
                  <h3 className="font-black text-slate-800 dark:text-white tracking-tight">Notifications</h3>
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Real-time updates</p>
                </div>
              </div>
              <button 
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {loading ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-8 space-y-4">
                  ...
                  <div>
                    <h4 className="font-black text-slate-800 dark:text-white text-sm uppercase tracking-tight">All caught up!</h4>
                    <p className="text-[10px] font-medium text-slate-400 dark:text-slate-500">No new notifications at the moment.</p>
                  </div>
                </div>
              ) : (
                notifications.map((n) => (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      n.read ? "bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800" : "bg-primary-blue/5 dark:bg-primary-blue/10 border-primary-blue/20 dark:border-primary-blue/30 shadow-sm"
                    )}
                  ...
                  >
                    <div className="flex gap-3">
                      <div className={cn(
                        ...
                        n.read ? "bg-slate-50 dark:bg-slate-800" : "bg-white dark:bg-slate-900 shadow-sm"
                      )}>
                        {getIcon(n.type)}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-2">
            ...
            <div className="p-6 border-t border-slate-100 dark:border-slate-800">
              <button 
                onClick={onClose}
                className="w-full py-4 bg-slate-900 dark:bg-primary-blue text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-slate-900/20 dark:shadow-primary-blue/20 active:scale-95 transition-all"
              >
                Close Panel
              </button>
            </div>
