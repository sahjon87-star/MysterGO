  ...
  return (
    <div className="p-6 space-y-6">
      <div className="space-y-1">
        <h2 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Provider Verification</h2>
        <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest">Review KYC documents</p>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[32px] p-16 text-center space-y-4 border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <div className="space-y-1">
              <h4 className="font-black uppercase tracking-tight text-sm text-slate-800 dark:text-white">No {activeTab} providers</h4>
              <p className="text-slate-500 dark:text-slate-400 text-[10px] font-medium">New submissions will appear here.</p>
            </div>
          </div>
        ) : (
          providers.map((provider) => (
            ...
            >
              ...
              <div className="w-16 h-16 rounded-[20px] bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-800 flex items-center justify-center overflow-hidden shadow-inner shrink-0 group-hover:scale-105 transition-transform">
                {provider.photoURL ? (
                  <img 
                  ...
                  <span className="text-slate-400 dark:text-slate-500 font-black text-xl">{getInitials(provider.name)}</span>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0 space-y-1">
                ...
                <div className="flex items-center gap-1.5 pt-1">
                  ...
                  <span className="text-[9px] font-medium text-slate-500 dark:text-slate-400 truncate max-w-[200px] italic">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center overflow-hidden border border-slate-100 dark:border-slate-800">
                    {selectedProvider.photoURL ? (
                      ...
                      <span className="text-slate-400 dark:text-slate-500 font-black text-sm">{getInitials(selectedProvider.name)}</span>
                    )}
                  </div>
                  <div>
                    <h3 className="text-lg font-black text-slate-800 dark:text-white tracking-tight">{selectedProvider.name}</h3>
                    <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{selectedProvider.phone}</p>
                  </div>
                </div>
                <button onClick={() => setSelectedProvider(null)} className="p-2 bg-slate-50 dark:bg-slate-800 rounded-xl"><X className="w-5 h-5 text-slate-400 dark:text-slate-500" /></button>
              </div>

              <div className="space-y-6">
                <div className="space-y-2">
                  <h4 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest px-2">NID Documents</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-center">Front</p>
                    ...
                    <div className="space-y-2">
                      <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-center">Back</p>
                      <div className="h-32 bg-slate-50 dark:bg-slate-800 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800">
                        <img 
                  ...
                  <div className="space-y-2 pt-2">
                    <p className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-center">Selfie with NID</p>
                    <div className="h-48 bg-slate-50 dark:bg-slate-800 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800">
                      <img 
                ...
                <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-4 space-y-1">
                  ...
                  <p className="text-sm font-black text-slate-800 dark:text-white tracking-widest">
