...
export const CustomerSearch: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-slate-950 transition-colors">
      {/* Search Header */}
      <div className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 p-4 space-y-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-blue outline-none text-sm font-medium dark:text-white"
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-full mr-2">
            <button 
              onClick={() => setViewMode('list')}
              className={`p-1.5 rounded-full transition-all ${viewMode === 'list' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 dark:text-slate-500'}`}
            >
              <ListIcon className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setViewMode('map')}
              className={`p-1.5 rounded-full transition-all ${viewMode === 'map' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 dark:text-slate-500'}`}
            >
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-full mr-2">
            <button 
              ...
              className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${searchType === 'workers' ? 'bg-primary-blue text-white shadow-sm' : 'text-slate-400 dark:text-slate-500'}`}
            >
              Workers
            </button>
            <button 
              onClick={() => setSearchType('shops')}
              className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${searchType === 'shops' ? 'bg-action-orange text-white shadow-sm' : 'text-slate-400 dark:text-slate-500'}`}
            >
              Shops
            </button>
          </div>
          ...
          {[
            ...
            <button 
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${activeFilter === f.id ? 'bg-primary-blue text-white shadow-md shadow-primary-blue/20 dark:shadow-none' : 'bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-slate-700'}`}
            >
              {f.label}
            </button>
          ))}
            ...
            >
              {loading ? (
                <div className="space-y-4 sm:space-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="flex gap-4 p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl animate-pulse">
                ...
                filteredWorkers.length === 0 ? (
                  <div className="text-center py-20 space-y-4">
                    ...
                    <h3 className="text-lg font-bold text-slate-800 dark:text-white">No workers found</h3>
                  ...
                  <div className="space-y-4 sm:space-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredWorkers.map((worker) => (
                      <motion.div 
                        ...
                        className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl hover:border-primary-blue/30 dark:hover:border-primary-blue/30 hover:shadow-md transition-all cursor-pointer group"
                      >
                        <div className="relative">
                          ...
                          {worker.isOnline && (
                            <div className="absolute -top-1 -right-1 w-4 h-4 bg-primary-blue border-4 border-white dark:border-slate-900 rounded-full" />
                          )}
                        </div>

                        <div className="flex-1 min-w-0">
                          <h3 className="font-bold text-slate-800 dark:text-white group-hover:text-primary-blue dark:group-hover:text-primary-blue transition-colors truncate">{worker.name || 'Anonymous Worker'}</h3>
                          <p className="text-slate-400 dark:text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-1">{worker.skill || worker.providerType || 'Service Provider'}</p>
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-1">
                              <Star className="w-3 h-3 fill-action-orange text-action-orange" />
                              <span className="text-xs font-bold text-slate-600 dark:text-slate-300">{(worker.rating || 0).toFixed(1)}</span>
                            </div>
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">per hour</div>
                        </div>
                      </motion.div>
                    ))}
                  </div>
