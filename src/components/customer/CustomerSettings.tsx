  ...
  return (
    <div className="space-y-8 bg-slate-50 dark:bg-slate-950 min-h-screen">
      <div className="px-4 pt-6 space-y-1">
        ...
        <h2 className="text-3xl font-black text-cream dark:text-cream uppercase tracking-tight">App Settings</h2>
        <p className="text-[10px] font-black text-gray-teal dark:text-gray-teal uppercase tracking-widest mt-1">Configure your MistriGO experience</p>
      </div>
      ...
      <div className="px-4 space-y-8 pb-32">
        ...
        <div className="bg-brand-slate dark:bg-brand-dark rounded-[40px] border border-slate-50 dark:border-slate-800 overflow-hidden shadow-sm">
           <button className="w-full flex items-center justify-between p-7 hover:bg-slate-50 dark:hover:bg-brand-surface transition-colors border-b border-slate-50 dark:border-slate-800">
              <div className="flex items-center gap-4">
                 <div className="w-10 h-10 bg-primary-blue/10 text-primary-blue rounded-xl flex items-center justify-center">
                    <MapPin className="w-5 h-5" />
                 </div>
                 <div className="text-left">
                    <span className="text-sm font-bold text-slate-700 dark:text-cream">Default Location</span>
                    <p className="text-[9px] font-black text-gray-teal dark:text-gray-teal uppercase tracking-widest mt-0.5">Current: {profile?.address || 'Not Set'}</p>
                 </div>
              </div>
           ...
           <div className="p-7 flex items-center justify-between border-b border-slate-50 dark:border-slate-800">
              <div className="flex items-center gap-4">
                 ...
                 <div className="text-left">
                    <span className="text-sm font-bold text-slate-700 dark:text-cream">Dark Appearance</span>
                    <p className="text-[9px] font-black text-gray-teal dark:text-gray-teal uppercase tracking-widest mt-0.5">Optimize for night use</p>
                 </div>
              </div>
              <ThemeToggle />
           </div>

           <button className="w-full flex items-center justify-between p-7 hover:bg-slate-50 dark:hover:bg-brand-surface transition-colors">
              <div className="flex items-center gap-4">
                 ...
                 <div className="text-left">
                    ...
                    <p className="text-[9px] font-black text-gray-teal dark:text-gray-teal uppercase tracking-widest mt-0.5">Enabled</p>
                 </div>
              </div>
              <ChevronRight className="w-5 h-5 text-cream" />
           </button>
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-black text-gray-teal uppercase tracking-[0.3em] px-4">Interface Language</h3>
          <div className="grid grid-cols-2 gap-4">
             {languages.map((l) => (
                <button
                  key={l.code}
                  onClick={() => setLang(l.code)}
                  className={`p-6 rounded-[32px] border-2 transition-all flex flex-col items-center gap-2 ${lang === l.code ? 'bg-primary-blue border-primary-blue text-cream shadow-xl shadow-primary-blue/20' : 'bg-brand-slate dark:bg-brand-dark border-slate-100 dark:border-slate-800 text-gray-teal'}`}
                >
        ...
        {/* Legal & Security */}
        <div className="space-y-4">
           <h3 className="text-[10px] font-black text-gray-teal uppercase tracking-[0.3em] px-4">Account & Safety</h3>
           <div className="bg-brand-slate dark:bg-brand-dark rounded-[40px] border border-slate-50 dark:border-slate-800 overflow-hidden shadow-sm">
             <button onClick={() => navigate('/terms')} className="w-full flex items-center justify-between p-6 border-b border-slate-50 dark:border-slate-800">
                <div className="flex items-center gap-4">
                   <Shield className="w-5 h-5 text-gray-teal" />
                   <span className="text-sm font-bold text-slate-700 dark:text-cream">Privacy Policy</span>
                </div>
                <ChevronRight className="w-5 h-5 text-cream" />
             </button>
             <button className="w-full flex items-center justify-between p-6">
                <div className="flex items-center gap-4">
                   <Lock className="w-5 h-5 text-gray-teal" />
                   <span className="text-sm font-bold text-slate-700 dark:text-cream">Security Credentials</span>
                </div>
                <ChevronRight className="w-5 h-5 text-cream" />
             </button>
           </div>
        ...
        {showDeleteModal && (
          <>
            ...
            >
              ...
              <h3 className="text-xl font-black text-cream dark:text-cream tracking-tight mb-2">Delete Customer Account?</h3>
