    ...
    <div className="space-y-6 pb-20">
        ...
        <div className="relative z-10 text-center space-y-6">
          ...
          <div className="flex gap-4 pt-4 max-w-sm mx-auto">
            <motion.button 
              ...
              className="flex-1 bg-primary-blue hover:bg-primary-light text-cream p-5 rounded-3xl shadow-xl shadow-primary-blue/30 transition-all flex flex-col items-center gap-2 border border-white/10"
            >
              <Plus className="w-6 h-6" />
              <span className="text-[10px] font-black uppercase tracking-widest text-cream/90">Deposit</span>
            </motion.button>
            <motion.button 
              whileTap={{ scale: 0.95 }}
              onClick={() => setShowWithdraw(true)}
              className="flex-1 bg-white/10 backdrop-blur-md hover:bg-white/20 text-cream p-5 rounded-3xl transition-all flex flex-col items-center gap-2 border border-white/10"
            >
              <ArrowUpRight className="w-6 h-6" />
              <span className="text-[10px] font-black uppercase tracking-widest text-cream/90">Payout</span>
            </motion.button>
          </div>
        </div>
      </div>
          ...
          <div className="relative z-10 space-y-5">
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-0.5">
                <h3 className="font-black text-base uppercase tracking-tight text-cream leading-none">Referral Bonus</h3>
                <p className="text-cream/70 text-[10px] font-bold uppercase tracking-widest">Earn ৳20 per successful referral</p>
              </div>
            </div>
      ...
      <div className="px-4 space-y-5">
        <div className="flex items-center justify-between px-2">
          <h3 className="font-black text-cream dark:text-cream uppercase tracking-widest text-xs">Transaction History</h3>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="glass-card p-16 text-center space-y-4">
              ...
              <p className="text-gray-teal dark:text-gray-teal text-[10px] font-black uppercase tracking-widest">No transactions yet</p>
            </div>
          ) : (
            sortedTransactions.map((tx) => (
              <motion.div 
                ...
                className="glass-card p-5 flex items-center gap-5 hover:bg-brand-slate dark:hover:bg-brand-dark transition-all border-slate-100 dark:border-slate-800 relative overflow-hidden"
              >
                ...
                <div className="text-right">
                  ...
                  <p className="text-[7px] font-black text-cream dark:text-slate-600 uppercase tracking-tighter">Settled</p>
                </div>
              </motion.div>
            ))
          )}
          ...
          <div className="fixed inset-0 z-[100] flex items-end justify-center p-4 bg-brand-dark/60 backdrop-blur-sm">
            ...
            >
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-black tracking-tight text-cream dark:text-cream uppercase">Add Money</h3>
                <button onClick={() => setShowAddMoney(false)} className="p-2 bg-slate-100 dark:bg-brand-surface rounded-xl">
              ...
              <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 scrollbar-hide">
                <div className="bg-primary-blue/10 p-5 rounded-[32px] border border-primary-blue/20 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="px-2 py-0.5 bg-primary-blue text-cream text-[8px] font-black rounded-full uppercase tracking-tighter">Personal</div>
                  </div>
                  <p className="text-xs font-bold text-slate-600 dark:text-cream leading-relaxed">
                  ...
                  <p className="text-[10px] text-gray-teal dark:text-gray-teal font-bold uppercase tracking-widest italic">Use "Wallet" as reference</p>
                </div>

                <div className="flex gap-2">
                  <button 
                    onClick={() => setMethod('bkash')}
                    className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${method === 'bkash' ? 'bg-pink-500 border-pink-500 text-cream shadow-lg shadow-pink-500/20' : 'bg-slate-50 dark:bg-brand-surface border-slate-100 dark:border-slate-700 text-gray-teal dark:text-gray-teal'}`}
                  >
                    bKash
                  </button>
                  <button 
                    onClick={() => setMethod('nagad')}
                    className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${method === 'nagad' ? 'bg-orange-500 border-orange-500 text-cream shadow-lg shadow-orange-500/20' : 'bg-slate-50 dark:bg-brand-surface border-slate-100 dark:border-slate-700 text-gray-teal dark:text-gray-teal'}`}
                  >
                    Nagad
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-teal uppercase tracking-widest px-2">Amount (৳)</label>
                  <input 
                    type="number" 
                    value={amount}
                    onChange={(e) => setAmount(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-brand-surface border border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-primary-blue outline-none text-lg font-black text-cream dark:text-cream"
                    placeholder="Enter amount"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-teal uppercase tracking-widest px-2">Transaction ID (TrxID)</label>
                  <input 
                    type="text" 
                    value={trxId}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-teal uppercase tracking-widest px-2">Payment Screenshot</label>
                  <div className="relative h-40 bg-slate-50 dark:bg-brand-surface border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden group flex flex-col items-center justify-center gap-2">
                    {screenshotPreview ? (
                      <img 
                      ...
                      <>
                        <Camera className="w-8 h-8 text-cream dark:text-slate-600" />
                        <span className="text-[10px] font-bold text-gray-teal dark:text-gray-teal uppercase tracking-widest">Tap to upload screenshot</span>
                      </>
                    )}
                    <input 
                      type="file" 
      ...
      <AnimatePresence>
        {showWithdraw && (
          <div className="fixed inset-0 z-[100] flex items-end justify-center p-4 bg-brand-dark/60 backdrop-blur-sm">
            ...
            >
              <div className="flex justify-between items-center">
                <h3 className="text-xl font-black tracking-tight text-cream dark:text-cream uppercase">Withdraw Money</h3>
                <button onClick={() => setShowWithdraw(false)} className="p-2 bg-slate-100 dark:bg-brand-surface rounded-xl">
                  <X className="w-5 h-5 text-gray-teal" />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  <button 
                    onClick={() => setMethod('bkash')}
                    className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${method === 'bkash' ? 'bg-pink-500 border-pink-500 text-cream shadow-lg shadow-pink-500/20' : 'bg-slate-50 dark:bg-brand-surface border-slate-100 dark:border-slate-700 text-gray-teal dark:text-gray-teal'}`}
                  >
                    bKash
                  </button>
                  <button 
                    onClick={() => setMethod('nagad')}
                    className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${method === 'nagad' ? 'bg-orange-500 border-orange-500 text-cream shadow-lg shadow-orange-500/20' : 'bg-slate-50 dark:bg-brand-surface border-slate-100 dark:border-slate-700 text-gray-teal dark:text-gray-teal'}`}
                  >
                    Nagad
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    value={amount}
                    onChange={(e) => setAmount(e.target.value)}
                    className="w-full bg-slate-50 dark:bg-brand-surface border border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-3.5 focus:ring-2 focus:ring-primary-blue outline-none text-sm font-bold text-cream dark:text-cream"
                    placeholder="Min ৳100"
                  />
                </div>
                ...
                <button 
                  ...
                  disabled={processing}
                  className="w-full bg-brand-dark dark:bg-primary-blue text-cream py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg flex items-center justify-center gap-2"
                >
