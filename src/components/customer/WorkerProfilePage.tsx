  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-32">
      ...
      <div className="bg-slate-900 px-4 pt-6 pb-12 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-5">
          ...
          <div className="space-y-1">
            ...
            <p className="text-slate-400 text-sm font-bold uppercase tracking-wider">{worker.skill || worker.providerType} • Dhaka</p>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{worker.totalJobs} Jobs Completed</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 -mt-6 relative z-20">
        <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-xl shadow-slate-200 dark:shadow-black/20 border border-slate-100 dark:border-slate-800 p-6 grid grid-cols-3 gap-4">
          <div className="text-center space-y-1 border-r border-slate-100 dark:border-slate-800">
            <div className="text-lg font-black text-slate-800 dark:text-white">{(worker.rating || 0).toFixed(1)}</div>
            <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Rating</div>
          </div>
          <div className="text-center space-y-1 border-r border-slate-100 dark:border-slate-800">
            <div className="text-lg font-black text-slate-800 dark:text-white">{worker.experience}+</div>
            <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Years Exp</div>
          </div>
          <div className="text-center space-y-1">
            <div className="text-lg font-black text-primary-blue">98%</div>
            <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tighter">Success</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-8 space-y-8">
        <div className="space-y-3">
          <h3 className="font-black text-slate-800 dark:text-white uppercase tracking-tight text-sm">About</h3>
          <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">{worker.bio || "No bio provided."}</p>
        </div>

        <div className="space-y-4">
          <h3 className="font-black text-slate-800 dark:text-white uppercase tracking-tight text-sm">Pricing & Work Type</h3>
          <div className="grid grid-cols-1 gap-3">
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                ...
                <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-xs">Standard Hourly Rate</div>
              </div>
            ...
            {worker.dailyRate && (
              <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-xs">Full Day (8 Hours)</div>
                </div>
            ...
            <div className="bg-slate-900 dark:bg-slate-900 rounded-3xl p-5 border border-slate-800 dark:border-slate-800 shadow-lg flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-600 font-medium">Worker's preferred way of working</p>
              </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-black text-slate-800 dark:text-white uppercase tracking-tight text-sm">Reviews ({worker.reviewCount || 0})</h3>
          ...
          <div className="space-y-4">
            {reviews.length > 0 ? (
              reviews.map((review) => (
                <div key={review.id} className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">
                        {getInitials(review.customerName)}
                      </div>
                      <div>
                        <div className="text-xs font-bold text-slate-800 dark:text-white">{review.customerName}</div>
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star 
                              key={s} 
                              className={`w-2.5 h-2.5 ${s <= review.rating ? 'fill-action-orange text-action-orange' : 'text-slate-200 dark:text-slate-800'}`} 
                            />
                          ))}
                        </div>
                      </div>
                  ...
                  {review.comment && (
                    <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed italic">"{review.comment}"</p>
                  )}
                </div>
              ))
            ) : (
              <div className="text-center py-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800">
                <p className="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">No reviews yet</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 w-full bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-t border-slate-100 dark:border-slate-800 p-4 pb-8 flex gap-4 z-50">
        <button 
          ...
          className="flex-shrink-0 w-14 h-14 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-2xl flex items-center justify-center transition-all active:scale-95"
        >
          <MessageCircle className="w-6 h-6" />
        </button>
        <button 
          onClick={() => navigate(`/new-booking/${worker.uid}`)}
          className="flex-1 bg-primary-blue hover:bg-primary-blue/90 text-white font-black py-4 rounded-2xl shadow-lg shadow-primary-blue/20 flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
        >
