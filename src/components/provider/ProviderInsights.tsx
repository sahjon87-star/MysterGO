...
export const ProviderInsights: React.FC = () => {
  ...
  const cards = [
    ...
    { label: 'Work Hours', value: '142h', icon: Clock, color: 'text-cream bg-slate-100 dark:bg-brand-surface' }
  ];
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="px-4 pt-6 space-y-1">
        ...
        <h2 className="text-3xl font-black text-cream dark:text-cream uppercase tracking-tight">Insights</h2>
        <p className="text-[10px] font-black text-gray-teal dark:text-gray-teal uppercase tracking-widest leading-none mt-1">Analyze your work performance</p>
      </div>
          ...
          <div className="relative z-10 space-y-8">
            <div className="flex items-center justify-between">
               <div className="space-y-1">
                 <p className="text-[9px] font-black text-gray-teal uppercase tracking-widest">Total Revenue</p>
                  ...
                  {['W', 'M', 'Y'].map((range) => (
                    <button 
                      ...
                      className={`w-8 h-8 rounded-full text-[9px] font-black uppercase flex items-center justify-center transition-all ${
                        ...
                        ? 'bg-primary-blue text-cream' : 'bg-white/10 text-gray-teal'
                      }`}
                    >
                      {range}
                    </button>
            ...
            <div className="h-40 flex items-end justify-between gap-2 px-2">
              {[40, 70, 45, 90, 65, 80, 55].map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2">
                  ...
                  <span className="text-[8px] font-black text-gray-teal tracking-tighter">Day {i+1}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="px-4 grid grid-cols-2 gap-4">
        {cards.map((card, i) => (
          <div key={i} className="bg-brand-slate dark:bg-brand-dark rounded-[32px] p-6 border border-slate-50 dark:border-slate-800 shadow-sm space-y-3 hover:border-primary-blue/20 transition-all">
             <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${card.color}`}>
                <card.icon className="w-5 h-5" />
             </div>
             <div className="space-y-0.5">
                <p className="text-lg font-black text-cream dark:text-cream tracking-tight leading-none">{card.value}</p>
                <p className="text-[8px] font-black text-gray-teal dark:text-gray-teal uppercase tracking-widest">{card.label}</p>
             </div>
          </div>
        ))}
      </div>
      ...
      <div className="px-4 space-y-4">
        <h3 className="text-[10px] font-black text-gray-teal dark:text-slate-600 uppercase tracking-[0.3em] px-4">Efficiency Metrics</h3>
        <div className="bg-brand-slate dark:bg-brand-dark rounded-[32px] border border-slate-50 dark:border-slate-800 shadow-sm overflow-hidden divide-y divide-slate-50 dark:divide-slate-800">
           <div className="flex items-center justify-between p-6">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <p className="text-xs font-black text-slate-700 dark:text-cream uppercase tracking-tight">Acceptance Rate</p>
                    <p className="text-[9px] font-bold text-gray-teal">Higher than 85% of peers</p>
                 </div>
              </div>
           ...
           <div className="flex items-center justify-between p-6">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <p className="text-xs font-black text-slate-700 dark:text-cream uppercase tracking-tight">Cancellation Rate</p>
                    <p className="text-[9px] font-bold text-gray-teal">Goal: Below 5%</p>
                 </div>
              </div>
           ...
           <div className="flex items-center justify-between p-6">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    <p className="text-xs font-black text-slate-700 dark:text-cream uppercase tracking-tight">Service Quality</p>
                    <p className="text-[9px] font-bold text-gray-teal">Based on recent 50 ratings</p>
                 </div>
              </div>
      ...
      <div className="px-4">
        <div className="bg-slate-50 dark:bg-brand-dark border-2 border-slate-100 dark:border-slate-800 rounded-[32px] p-8 space-y-4">
          <div className="flex items-center justify-between">
             <h4 className="text-xs font-black text-cream dark:text-cream uppercase tracking-tight">Monthly Target</h4>
             <span className="text-[10px] font-bold text-gray-teal">৳10,000 / ৳25,000</span>
          </div>
          <div className="h-3 bg-slate-200 dark:bg-brand-surface rounded-full overflow-hidden">
             <motion.div 
               initial={{ width: 0 }}
               animate={{ width: '40%' }}
          ...
          <p className="text-[10px] font-bold text-gray-teal text-center uppercase tracking-widest">You're 40% towards your goal!</p>
        </div>
      </div>
    </div>
  );
