  ...
  return (
    <div className="space-y-10 pb-20 transition-all duration-500">
      ...
      <div className="bg-slate-950 px-6 pt-12 pb-24 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between mb-10">
          <div className="space-y-1.5 px-2">
            <p className="text-gray-teal text-[9px] font-black uppercase tracking-[0.4em] leading-none">Commerce Overview</p>
            <div className="flex items-center gap-3">
              <h2 className="text-3xl font-black text-cream tracking-tighter uppercase leading-none">{profile?.shopName || profile?.name}</h2>
              {profile?.isVerified && (
          ...
          >
            {profile?.photoURL ? (
              ...
              <Store className="w-8 h-8 text-cream opacity-40" />
            )}
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-5 relative z-10">
          {stats.map((stat, i) => (
            <motion.div 
              ...
              className="bg-white/5 dark:bg-brand-dark/40 backdrop-blur-3xl border border-white/10 rounded-[28px] p-5 space-y-3 shadow-glass"
            >
      ...
      {!profile?.isOpen && (
        ...
        >
          <div className="bg-brand-dark/80 backdrop-blur-3xl rounded-[40px] p-8 border border-white/5 shadow-2xl flex items-center justify-between group overflow-hidden">
            ...
            <div className="space-y-1 relative z-10">
              ...
              <p className="text-[9px] font-bold text-gray-teal uppercase tracking-widest leading-relaxed">Commerce node is currently offline.</p>
            </div>
            <motion.button 
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => navigate('/merchant/settings')}
              className="bg-action-orange text-cream px-6 py-3 rounded-2xl text-[9px] font-black uppercase tracking-[0.3em] shadow-lg shadow-action-orange/20 relative z-10 transition-all hover:bg-brand-slate hover:text-action-orange"
            >
              Initialize Store
            </motion.button>
          </div>
      ...
      <section className="px-6 grid grid-cols-2 gap-5">
        <motion.button 
          ...
          className="bg-brand-slate dark:bg-brand-dark p-8 rounded-[40px] border border-slate-100 dark:border-white/5 shadow-glass flex flex-col gap-5 text-left group transition-all"
        >
          ...
          <div>
            <h4 className="text-[10px] font-black text-cream dark:text-cream uppercase tracking-[0.3em]">Deploy SKU</h4>
            <p className="text-[9px] font-black text-gray-teal uppercase tracking-widest mt-1">Append Catalog</p>
          </div>
        </motion.button>
        <motion.button 
          whileHover={{ y: -5 }}
        ...
        >
          ...
          <div>
            <h4 className="text-[10px] font-black text-cream dark:text-cream uppercase tracking-[0.3em]">Capital Core</h4>
      ...
      <section className="space-y-6">
        <div className="px-8 flex items-center justify-between">
          <h3 className="font-black text-cream dark:text-cream uppercase tracking-[0.3em] text-[11px] flex items-center gap-3">
        ...
        <div className="px-6 space-y-5">
          {loading ? (
             <div className="h-24 bg-slate-100 dark:bg-brand-dark rounded-[40px] animate-pulse" />
          ) : activeOrders.length === 0 ? (
            <div className="text-center py-20 bg-white/50 dark:bg-brand-dark/30 backdrop-blur-xl rounded-[48px] border border-slate-100 dark:white/5 space-y-6 grayscale opacity-50">
              <div className="w-20 h-20 bg-slate-100 dark:bg-white/5 rounded-full flex items-center justify-center mx-auto text-cream shadow-inner">
                <ShoppingBag className="w-10 h-10" />
              </div>
              <p className="text-cream dark:text-cream font-black text-sm uppercase tracking-widest">No Active Orders</p>
            </div>
          ) : (
            <div className="space-y-4">
              {activeOrders.map((order) => (
                <motion.div 
                  ...
                  className="bg-brand-slate dark:bg-brand-dark rounded-[40px] p-6 border border-slate-100 dark:border-white/5 shadow-glass flex items-center gap-5 hover:border-primary-blue/30 transition-all cursor-pointer group relative overflow-hidden"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-black text-cream dark:text-cream group-hover:text-primary-blue transition-colors truncate uppercase tracking-tighter text-lg leading-none">{order.customerName}</h4>
                    <div className="flex items-center gap-3 mt-2">
                      <p className="text-gray-teal text-[8px] font-black uppercase tracking-[0.2em]">ID: #{order.id?.slice(-5)}</p>
                  ...
                  <div className="text-right space-y-1 shrink-0">
                    <div className="text-xl font-black text-cream dark:text-cream tracking-tighter leading-none">{formatCurrency(order.totalAmount)}</div>
                    <div className="text-[8px] font-black text-gray-teal uppercase tracking-widest">{order.time}</div>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <section className="space-y-6">
          ...
          <div className="px-6 flex gap-4 overflow-x-auto pb-4 scrollbar-hide no-scrollbar pr-10">
            {lowStockProducts.map((p) => (
              ...
              >
                 <div className="w-14 h-14 bg-brand-slate dark:bg-brand-surface rounded-2xl overflow-hidden flex items-center justify-center border border-red-100 shrink-0 shadow-2xl">
                   <img src={p.image} className="w-full h-full object-cover" alt={p.name} />
                 </div>
                 <div className="min-w-0">
                    <h5 className="text-[9px] font-black text-cream dark:text-cream uppercase truncate tracking-[0.1em] leading-none mb-1.5">{p.name}</h5>
      ...
      <section className="px-6">
        <motion.div 
          ...
          className="bg-slate-950 rounded-[56px] p-12 text-cream relative overflow-hidden shadow-[0_40px_80px_-20px_rgba(0,0,0,0.5)] group active:scale-[0.99] transition-all cursor-pointer"
        >
          ...
          <div className="relative z-10 space-y-8">
            <div className="flex items-center justify-between">
              ...
              <div className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest text-gray-teal">Node Cycle: Weekly</div>
            </div>
            
            <div className="grid grid-cols-2 gap-12">
               <div className="space-y-3">
                 <p className="text-[9px] font-black text-gray-teal uppercase tracking-[0.4em]">Matrix Revenue</p>
                 <div className="text-4xl font-black tracking-tighter text-cream">{formatCurrency(profile?.walletBalance || 0)}</div>
               </div>
               <div className="space-y-3">
                 <p className="text-[9px] font-black text-gray-teal uppercase tracking-[0.4em]">Signal Impressions</p>
                 <div className="text-4xl font-black tracking-tighter text-cream">12.4k</div>
               </div>
            </div>
